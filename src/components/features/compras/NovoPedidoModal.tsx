...
import { useCriarPedido } from "@/hooks/compras/useMutationsPedido";
...
export function NovoPedidoModal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Gerar número aleatório para o exemplo
    ...
    const pedido = {
      ...
      status_liberacao: 'pendente' as any,
      previsao_entrega: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    };
    ...
    criarPedido({ pedido, itens: itensParaSalvar }, {
      onSuccess: () => {
        setOpen(false);
        setFornecedorId("");
        setItens([]);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="text-xs">
          <Plus className="mr-1 h-3 w-3" /> Novo Pedido
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Forjar Novo Pedido de Compra</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending || !fornecedorId || itens.length === 0}>
              {isPending ? "Forjando..." : "Salvar Rascunho"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
