...
import { generateCspNonce, withCspNonce } from "./lib/security/nonce";
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    return withCspNonce(generateCspNonce(), async () => {
      try {
        if (!isCsrfSafe(request)) {
          return applySecurityHeaders(
            new Response("Acesso negado: Falha na verificação CSRF", {
              status: 403,
