...
export function NovoOrcamentoModal() {
  ...
  const { createOrcamento, isCreating } = useOrcamentoMutations();
  const {
    tiposVidro,
    processamentos,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const numero = `ORC-${Math.floor(1000 + Math.random() * 9000)}`;
  ...
  const handleAdicionarProduto = () => {
    ...
    // Verificar se é um vidro avulso (prefixo "vidro:")
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="text-xs">
          <Plus className="mr-1 h-3 w-3" /> Novo orçamento
        </Button>
      </DialogTrigger>
        ...
        {loadingProdutos ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5 py-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="descricao">Descrição / Projeto</Label>
                <Input
                  id="descricao"
            ...
            <DialogFooter className="gap-2 sm:gap-0">
              ...
              >
                {isCreating ? "Criando..." : "Gerar Orçamento"}
              </Button>
            </DialogFooter>
          </form>
        )}
